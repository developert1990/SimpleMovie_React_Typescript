...
import bcrypt from 'bcrypt'; // hash 로 salting 해서 password를 암호화하여 저장하기 위한 API 호출
...
app.get('/movie/findMoviesByTitle/:title', (req, res) => {
    ...
    // console.log(title)
    const matchedData = title.length > 0 && data.movies.filter((movie: movieDataType) => movie.title.includes(title));
    console.log(title.length);
    matchedData ? res.json({ data: matchedData }) : res.status(404).send('No movie founded');
...
app.get('/movie/findMoviesByYear/:year', (req, res) => {
    ...
    const matchedData = data.movies.filter((movie: movieDataType) => movie.year === year);
    console.log(matchedData);
    matchedData ? res.json({ data: matchedData }) : res.status(404).send('No movie founded');
});
...
app.get('/movie/findMoviesByGenre/:genre', (req, res) => {
    ...
    console.log(genre);
    const matchedData = data.movies.filter((movie: movieDataType) => movie.genres.map(g => g.toLowerCase().trim()).includes(genre))
    console.log(matchedData);
...
const createData = async (body: bodyInfoType) => {
...
const getClickedData = async (id: string) => {
    const QUERY = `SELECT * FROM mflixboard WHERE id=?`;
    const [rows] = await promisePool.query(QUERY, id);
    return rows;// 어차피 하나만 불러오기 때문에 상관없고 [{},{},{}]이런식으로 배열로 받아온다 그래서 첫번째만 받으면 되므로 이렇게 받아주는게 프론트단에서 깔끔하다
}
...
const updateData = async (bodyInfo: bodyInfoType) => {
...
const getDataByTitle = async (title: string) => {
...
app.post('/movie/login', async (req, res, next) => {
...
const getPasswordHashByUsername = async (username: string) => {
    console.log('비교하러 들어와봄', username)
    const QUERY = "SELECT password FROM simplemovie_logindb where username = ?";
    const [rows] = await promisePool.query(QUERY, username);
    // console.log('비번 값: ', rows[0].password);
    // return rows.length == 0 ? 0 : rows[0].password;

    return rows ? (<rowType[]>rows)[0].password : null;
    // return 'gg';
}
...
const usernameDoesExist = async (username: string) => {
    const QUERY = 'SELECT username FROM simplemovie_logindb where username = ?';
    const [rows] = await promisePool.query(QUERY, username);
    return rows ? (<rowType[]>rows)[0].length > 0 : null
...
const storeUserInfo = async (username: string, password: string) => {
